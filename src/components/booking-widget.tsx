...
const WHATSAPP_NUMBER =
  process.env.NEXT_PUBLIC_WHATSAPP_NUMBER ?? "31612345678"
...
function makeWhatsappLink(params: {
  ...
  const voertuig =
    params.vehicleType === "taxibus"
      ? "Taxibus (5-8 personen)"
      : "Taxi (1-4 personen)"
  const text = [
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Tarief kon niet worden berekend. Probeer opnieuw."
      )
    } finally {
      setLoading(false)
    }
  ...
  async function handleBook() {
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Boeking mislukt. Gebruik WhatsApp als alternatief."
      )
    } finally {
      setLoading(false)
    }
      ...
      <div className="rounded-2xl border border-white/[0.09] bg-[#111111] p-6 shadow-2xl">
        ...
        <div className="mb-4 rounded-xl border border-white/[0.07] bg-white/[0.03] p-3.5 text-xs">
          ...
          <p className="mt-2.5 text-white/30">
            {priceResult.distanceKm} km ·&nbsp;ca. {priceResult.durationMin} min
          </p>
        </div>
        ...
        <div className="mb-4 rounded-xl border border-[#D4B896]/[0.15] bg-[#D4B896]/[0.05] p-4">
          ...
          <p className="text-3xl font-black text-[#D4B896]">
            € {priceResult.price.toFixed(2).replace(".", ",")}
          </p>
          <p className="mt-1 text-[10px] text-white/25">
            Starttarief €{priceResult.starttarief.toFixed(2)} + {priceResult.distanceKm} km × €
            {priceResult.kmTarief.toFixed(2)}
            {vehicleType === "taxibus" ? " (taxibus)" : ""}
          </p>
        </div>
        ...
        >
          {loading ? (
            ...
            <>
              Boeking bevestigen & betalen <ArrowRight className="size-4" />
            </>
          )}
        </Button>

        <a
          href={makeWhatsappLink({ origin, destination, date, time, vehicleType, name })}
          target="_blank"
  ...
  return (
    <div className="rounded-2xl border border-white/[0.09] bg-[#111111] p-6 shadow-2xl">
      ...
      <div className="space-y-2.5">
        ...
        {/* Voertuigkeuze */}
        <div className="grid grid-cols-2 gap-2 rounded-xl border border-white/[0.07] bg-white/[0.03] p-1">
          {(["taxi", "taxibus"] as const).map((v) => (
            <button
              key={v}
              onClick={() => setVehicleType(v)}
              className={cn(
                "rounded-lg py-2.5 px-3 text-left transition-all duration-200",
                vehicleType === v
                  ? "bg-[#D4B896] shadow-sm"
                  : "hover:bg-white/[0.05]"
              )}
            >
              <span
                className={cn(
                  ...
                  vehicleType === v ? "text-[#0d0c0b]" : "text-white/60"
                )}
              >
                {v === "taxi" ? "Taxi" : "Taxibus"}
              </span>
              <span
                className={cn(
                  "text-xs",
                  vehicleType === v ? "text-[#0d0c0b]/50" : "text-white/30"
                )}
              >
                {v === "taxi" ? "1 tot 4 personen" : "5 tot 8 personen"}
              </span>
            </button>
          ))}
        </div>
      ...
      <p className="mt-4 text-center text-[11px] text-white/25 tracking-wide">
        &#9733;&nbsp;4,9/5 beoordeling &nbsp;·&nbsp; Pin &nbsp;·&nbsp; Creditcard
        &nbsp;·&nbsp; Contant
      </p>
    </div>
  )
