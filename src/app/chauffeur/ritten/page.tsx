    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        {todayRides.length === 0 ? (
          ...
          <div className="space-y-3">{todayRides.map((ride) => <RideCard key={ride.id} ride={ride} highlight />)}</div>
        )}
      </div>
      ...
      {upcomingRides.length > 0 && (
        <div>
          ...
          <div className="space-y-3">{upcomingRides.map((ride) => <RideCard key={ride.id} ride={ride} />)}</div>
        </div>
      )}

      {all.length === 0 && (
        <EmptyState label="Geen toegewezen ritten gevonden." sub="Nieuwe ritten verschijnen hier zodra de planner een rit aan u toewijst." />
      )}
    </div>
  )
}
